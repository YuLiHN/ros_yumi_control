...
#include "hackathon/PickPlaceRequest.h"
...
public:
	...
	YumiArmController(ros::NodeHandle& nh, int arm_id) : nh_(nh), arm_id_(arm_id) {
		...
		desired_pose_subscriber = nh_.subscribe("/topic_base/pick_pose",1, &YumiArmController::desired_pose_callback, this);
	...
	void close_gripper() {
		if (arm_id_ == 1) {  // left arm
			...
			{	
				...
				std::cout<<current_gripper - right_gripper<<std::endl;

				if(current_gripper - right_gripper<0.0003 || gripper_cmd.data==0.)
				{
					closed=true;
				}
	...
	void pick_object(KDL::Frame& desired_pose)
	{
	...
	bool pick_object_callback(hackathon::PickPlaceRequest::Request  &req,
			hackathon::PickPlaceRequest::Response &res)
	{
		...
		double z = current_pose.p(2);
		KDL::Vector desired_position(desired_geometry_msg.pose.position.x, desired_geometry_msg.pose.position.y, z);
		...
		pick_object(desired_pose);
		place_object();
		res.success=true;
		return true;
	}

	bool place_object_callback(hackathon::PickPlaceRequest::Request  &req,
			hackathon::PickPlaceRequest::Response &res)
	{
		//    res.success = req.a + req.b;
		//    ROS_INFO("request: x=%ld, y=%ld", (long int)req.a, (long int)req.b);
	...
	bool teleop_srv_callback(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
	{
		flag_teleop=true;
		while (ros::ok())
		{
	...
	{	
		...
		kdl_twist.rot.x(0.25*ROTATION_CONST* angle1_diff);
		kdl_twist.rot.y(0.25*ROTATION_CONST* angle2_diff);
		kdl_twist.rot.z(0.25*ROTATION_CONST* angle3_diff);
		...
		if (arm_id_==2)
		{
			gripper_cmd.data = (1-press_middle)*20;
			gripper_command_pub.publish(gripper_cmd);
		}
		else
		{
